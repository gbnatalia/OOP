...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        PrintToConsole act1 = new PrintToConsole();

        // создание объекта класса вывода информации по узлу дерева родословной
        // определение состава выводимой информации
        Print_GT_Data pgt = new Print_GT_Data(act1);
        pgt.addInterface(new PrintPersonInfo(act1));
        pgt.addInterface(new PrintParentsTree(tree, act1));
        pgt.addInterface(new PrintChildTree(tree, act1));
        pgt.addInterface(new PrintBrotherAndSister(tree, act1));
        ...
        PrintToTxtFile act2 = new PrintToTxtFile("MyGeoTree");
        pgt.changeStream(act2);
        if (needPerson != null)
            pgt.print(needPerson);
        else
            System.out.printf("В дереве нет информации о персоне \"%s\" %d года рождения", "субъект", 5555);
